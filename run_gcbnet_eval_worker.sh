...
OUT_DIR="${OUT_DIR:-/root/autodl-tmp/results/seed_gcbnet_eval}"
GPU="${GPU:-0}"
EPOCHS="${EPOCHS:-200}"
...
for subj in "$@"; do
  ...
  if [[ ! -f "${syn_path}" ]]; then
    echo "[s${subj}] missing synthetic file: ${syn_path}" |
      tee -a "${OUT_DIR}/missing.log"
    continue
  fi

  echo "================================================================"
  echo "[s${subj}] GCBNet source-only vs KNN0.1 synthetic"
  echo "log: ${log_path}"
  echo "================================================================"
