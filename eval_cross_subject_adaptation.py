...
def main():
    ...
    parser.add_argument("--pseudo_path", required=True)
    ...
    requested_syn = int(len(source_fit_y) * args.syn_ratio)
    pseudo_count = min(requested_pseudo, len(pseudo_y))
    syn_count = min(requested_syn, len(syn_y))
    if pseudo_count < 1 or syn_count < 1:
        raise ValueError("adaptation sample count is zero")
    ...
    methods = [
        ("source_only", source_fit_x, source_fit_y),
        ("source+pseudo", np.concatenate([source_fit_x, pseudo_x]), np.concatenate([source_fit_y, pseudo_y])),
        ("source+target_adapted_synthetic", np.concatenate([source_fit_x, syn_x]), np.concatenate([source_fit_y, syn_y])),
    ]
    ...
    for name, train_x, train_y in methods:
        ...
        for run in range(args.n_runs):
            print(f"  starting run={run + 1}/{args.n_runs}, epochs<={args.epochs}, batch={args.batch_size}, val_interval={args.val_interval}", flush=True)
            ...
            result = train_and_evaluate(
                ...
                val_data=source_val_x, val_labels=source_val_y,
            )
